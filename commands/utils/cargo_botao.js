  ...
  run: async (client, interaction) => {
    if (
      ...
      let embed = new Discord.EmbedBuilder()
        ...
        .setDescription(`Clique no botão abaixo para liberar o servidor`);
      ...
      interaction.reply({ embeds: [embed], components: [botao] }).then(() => {
        ...
        coletor.on("collect", (c) => {
          if (!c.member.roles.cache.get(cargo.id)) {
            ...
            c.reply({
              content: `Olá **${c.user.username}**, você se verificou com sucesso **${cargo.name}**.`,
              ephemeral: true,
            });
          } else if (c.member.roles.cache.get(cargo.id)) {
            c.member.roles.remove(cargo.id);
            c.reply({
              content: `Olá **${c.user.username}**, você se desverificou com sucesso **${cargo.name}**.`,
              ephemeral: true,
            });
          }
        });
